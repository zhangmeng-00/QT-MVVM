...
classDiagram
    ...
    class Observe {
        <<interface>>
        +ObserveData(tag: QString, value: QVariant)
    }

    class Mediator {
        -TopicRegistry m_registry
        +ConnectObserve(obs: Observe*)
        +Publish(tag: QString, value: QVariant)
    }

    class TopicRegistry {
        -Map<QString, Topic> m_topics
        +Register(tag: QString, meta: TopicMeta)
        +GetOrCreate(tag: QString) Topic*
        +Has(tag: QString) bool
    }

    class Topic {
        -QString tag
        -TopicMeta meta
        -List<Observe*> subscribers
        +Subscribe(obs: Observe*)
        +Unsubscribe(obs: Observe*)
        +Notify(value: QVariant)
    }
    ...
    %% relationships
    Observe <|.. ConcreteObserve
    Mediator --> TopicRegistry : owns
    TopicRegistry --> Topic : manages
    Topic --> Observe : notifies
    Topic --> TopicMeta : has
    TopicMeta --> TopicKind
